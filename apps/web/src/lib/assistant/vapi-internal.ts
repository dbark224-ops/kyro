...
export async function getVapiInternalVoiceSession({
  ...
  const contextMessage = [
    ...
    "For contact profile changes, call kyro_update_contact. It can update contact name, email, phone, another number, address, company, contact type, and append or replace notes. If the contact is unclear, call kyro_lookup_contact first and ask the user to pick.",
    "When a number belongs to somebody else -- a partner, an assistant, whoever is actually on site -- set secondaryPhone, their name in secondaryPhoneName, and what they are to the customer in secondaryPhoneLabel. Never overwrite the contact's own phone with it.",
