...
function parseSettingChanges(prompt: string): ParsedSettingChange {
  ...
  if (text.includes("fetch cap") || text.includes("max messages") || text.includes("message cap")) {
    const maxMessages = numberNear(text, /\b(\d{1,2})\b/);
...
export function looksLikeSettingsUpdatePrompt(prompt: string) {
  ...
  const hasSettingTarget =
    /\b(timezone|time zone|quiet|poll|polling|sync mode|manual only|lookback|fetch cap|max messages|skipped|awareness|summaries)\b/.test(text);
...
export async function updateAssistantEditableSettings({
  ...
  const parsed = parseSettingChanges(prompt);
  ...
  if (parsed.labels.length === 0) {
    return {
      ...
      fallbackAnswer:
        "I can edit safe settings like timezone, email polling, quiet hours, missed-mail lookback, fetch cap, and skipped-mail summaries. I could not confidently read the new value from that request.",
  ...
  return {
    context: {
      changed: parsed.labels,
      settingsSnapshot: {
        includeAwarenessEvents: settings.includeAwarenessEvents,
