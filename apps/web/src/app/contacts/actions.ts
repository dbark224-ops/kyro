...
function leadError(message: string): never {
  redirect(
    `/contacts?filter=opportunities&engine_error=${encodeURIComponent(message)}`,
  );
}
...
export async function createManualLeadAction(formData: FormData) {
  ...
  try {
    ...
    leadError(
      error instanceof Error ? error.message : "Unable to match contact.",
    );
  }
  ...
  if (!contactId && hasContactDetails) {
    ...
    const { data: contact, error } = await supabase
      ...
      .insert({
        ...
        // A contact created alongside a new CRM lead is a lead. Kyro promotes
        // it to client on its own once a quote is approved or a job is paid.
        contact_type: "lead",
        email,
...
export async function runContactLifecycleReviewAction(formData: FormData) {
  ...
  redirectWithStatus(
    redirectTo,
    "engine_message",
    `Reviewed ${summary.reviewed} contact${
      summary.reviewed === 1 ? "" : "s"
    }: ${summary.promoted} promoted to client.`,
  );
}
