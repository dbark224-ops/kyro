...
3. Assistant
   ...
   - When Assistant returns an `outbound_call_request` block, render a compact review card with recipient, phone number, call instructions, and a Start call action that posts the same payload to `POST /api/voice/outbound`.
